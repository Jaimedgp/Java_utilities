...
/**
 * Clase que almacena una lista de medidas realizadas por un pluviometro y 
...
public class DatosPluviometro
{
    ...
    public DatosPluviometro(String nomFich) {
        ...
        try {
            ...
            lista=listaTemporal.toArray(new MedidaPrecip[listaTemporal.size()]);

        } catch (FileNotFoundException e) {
            System.out.println("No se encuentra el fichero " + nomFich);
            lista=new MedidaPrecip[0]; // creamos el array vacio
        } finally {
            if (in != null)
    ...
    public void listarMes(int mes, int year) {
        ...
        for (int i = 0; i<lista.length ; i++) {
            ...
            if (lista[i].getYear()==year && lista[i].getMes()==mes) {
                System.out.println(lista[i].getDia() + "\t" + precipitacionDia(lista[i].getDia(), mes, year));
                i+=17;
            }           
        } 
    }
    ...
    public double precipitacionMaxEnMes(int mes) {
        ...
        for (int i=0; i<lista.length; i++) {
            if (lista[i].getMes()==mes && lista[i].getPrecipHora()>precipitacion) {
